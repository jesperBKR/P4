 ...
 #   """ Argument parser """
  #  parser = argparse.ArgumentParser(description='Drive fingers to command position')
   # parser.add_argument('kinova_robotType', metavar='kinova_robotType', type=str, default='j2n6a300')
    ...
    #parser.add_argument('unit', metavar='unit', type=str, default='percent',
...
if __name__ == '__main__':
    ...
    finger_val = [100 ,50,-50]

    kinova_robotTypeParser('j2n6s300')
    rospy.init_node(prefix + 'gripper_workout')
#    if len(finger_value) != finger_number:
#        print('Number of input values {} is not equal to number of fingers {}. Please run help to check number of fingers with different robot type.'.format(len(3), finger_number))
#        sys.exit(0)

    # get Current finger position if relative position
    getCurrentFingerPosition(prefix)

    finger_turn, finger_meter, finger_percent = unitParser('percent', finger_val, True )
