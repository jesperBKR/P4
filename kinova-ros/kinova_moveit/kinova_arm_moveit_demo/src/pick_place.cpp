...
//    const robot_state::JointModelGroup *joint_model_group = robot_state.getJointModelGroup("arm");
    ...
    /*pose_client_ = new actionlib::SimpleActionClient<kinova_msgs::ArmPoseAction>
            ("/" + robot_type_ + "_driver/arm_action/cartesian_pose", false);
    while(robot_connected_ && !pose_client_->waitForServer(ros::Duration(5.0))){
      ROS_INFO("Waiting for the cartesian_pose action server to come up");
    }*/
...
bool PickPlace::cartesian_action(geometry_msgs::PoseStamped pose){
  kinova_msgs::ArmPoseGoal goal;
  tf::Quaternion q;
  q = EulerZYZ_to_Quaternion(-M_PI/4, M_PI/2, M_PI);
  goal.pose.header.frame_id    = start_pose_.header.frame_id = "root";//pose.header.frame_id;
  goal.pose.header.stamp       = start_pose_.header.stamp = ros::Time::now();//pose.header.stamp;
  goal.pose.pose.position.x    = start_pose_.pose.position.x = 0.4;//pose.pose.position.x;
  goal.pose.pose.position.y    = start_pose_.pose.position.y = -0,5;//pose.pose.position.y;
  goal.pose.pose.position.z    = start_pose_.pose.position.z = 0.5; //pose.pose.position.z;
  goal.pose.pose.orientation.x = start_pose_.pose.orientation.x = q.x();//pose.pose.orientation.x;
  goal.pose.pose.orientation.y = start_pose_.pose.orientation.y = q.y();//pose.pose.orientation.y;
  goal.pose.pose.orientation.z = start_pose_.pose.orientation.z = q.z();//pose.pose.orientation.z;
  goal.pose.pose.orientation.w = start_pose_.pose.orientation.w = q.w();//pose.pose.orientation.w;
  pose_client_->sendGoal(goal);

  if (pose_client_->waitForResult(ros::Duration(5.0)))
  {
  ...
  {
      pose_client_->cancelAllGoals();
      ROS_WARN_STREAM("The cartesian action timed-out");
      return false;
  }
}
...
void PickPlace::define_cartesian_pose(){
    ...
    start_pose_.pose.position.x = 0.9;
    start_pose_.pose.position.y = 0.08;
    start_pose_.pose.position.z = 0.18;

    q = EulerZYZ_to_Quaternion(-M_PI/4, M_PI/2, M_PI);
    start_pose_.pose.orientation.x = 0.67;
    start_pose_.pose.orientation.y = -0.21;
    start_pose_.pose.orientation.z = 0.72;
    start_pose_.pose.orientation.w = -0.11;

    // next pose
    next_pose_.header.frame_id = "root";
    next_pose_.header.stamp = ros::Time::now();
    next_pose_.pose.position.x += 0.5;
    next_pose_.pose.position.y = -0.5;
    next_pose_.pose.position.z += 0.5;

    q = EulerZYZ_to_Quaternion(-M_PI/4, M_PI/2, M_PI);
    next_pose_.pose.orientation.x = q.x();
    next_pose_.pose.orientation.y = q.y();
    next_pose_.pose.orientation.z = q.z();
    next_pose_.pose.orientation.w = q.w();
    ...
    grasp_pose_.pose.position.z = 0.4;
 ...
 * @param rot_gripper_z rotation along thegenerate_gripper_align_pose z axis of the gripper reference frame (last joint rotation)
...
*/
    ...
    evaluate_plan(*group_);
    //group_->move();
    //cartesian_action(start_pose_);
    //ROS_INFO_STREAM("Moving to next target ");
    //group_->setPoseTarget(next_pose_);
    //evaluate_plan(*group_);
    //group_->move();
    //cartesian_action(next_pose_);
