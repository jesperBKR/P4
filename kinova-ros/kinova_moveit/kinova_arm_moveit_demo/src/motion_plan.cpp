...
int main(int argc, char **argv)
{
  ...
  //tf::Pose Home;
  //Home.setOrigin(tf::Vector3(0.07,0.48,0.21));
  //Home.setRotation(kinova::EulerXYZ2Quaternion(45,90,180));//1.63771402836,1.11316478252, 0.134094119072));
  ...
  start_pose.pose.position.x = 0.9;
  start_pose.pose.position.y = 0.08;
  start_pose.pose.position.z = 0.18;
  ...
  std::string pause;
  tf::Pose random_pose;
  random_pose.setOrigin(tf::Vector3(0.90, 0.08,  0.18));
  random_pose.setRotation(tf::Quaternion(0.67 , -0.21, 0.72, -0.11));
...
/*
  ...
  ROS_INFO("Press any button to procced.");
  std::cin >>pause;
  tf::Pose random_pose2;
  random_pose2.setOrigin(tf::Vector3(box_pose.position.x-0.03,box_pose.position.y,box_pose.position.z));
  ...
  success = static_cast<bool>(group.plan(my_plan2));
  ROS_INFO("Moving on x - axis %s",success?"":"FAILED");
